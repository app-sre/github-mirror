...
class _GithubStatus:
    ...
    def _is_github_online(response):
        ...
        return any(
            c["name"] == "API Requests" and c["status"] != "major_outage"
            for c in components
        )
    ...
    def check(self):
        ...
        while True:
            try:
                response = self.session.get(GH_STATUS_API, timeout=STATUS_TIMEOUT)
                response.raise_for_status()
                self.online = self._is_github_online(response)
                if not self.online:
                    LOG.warning("Github API is offline, response: %s", response.text)
            except (
