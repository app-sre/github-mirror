...
class MockResponse:
    def __init__(self, content, headers, status_code, user=None, json_content=None):
...
def test_mirror_upstream_call(mock_monitor_session, mocked_request, client):
    ...
    client.get("/user/repos?page=2", headers={"Authorization": "foo"})
...
def test_mirror_user_forbidden(mock_monitor_session, _mocked_cond_request, client):
    ...
    response = client.get(
        "/repos/app-sre/github-mirror", headers={"Authorization": "foo"}
    )
...
def test_mirror_no_auth(mock_monitor_session, client):
    ...
    response = client.get("/repos/app-sre/github-mirror", headers={})
...
def test_offline_mode(mock_monitor_session, _mock_request, client):
    ...
    assert response.status_code == 200
    assert (
        'request_latency_seconds_count{cache="OFFLINE_HIT",'
        'method="GET",status="200",user="None"} 1.0'
    ) in str(response.data)
    ...
    assert response.status_code == 200
    assert (
        'request_latency_seconds_count{cache="OFFLINE_MISS",'
        'method="GET",status="504",user="None"} 1.0'
    ) in str(response.data)

    # POST, just to check if we are behaving.
    response = client.post("/repos/app-sre/github-mirror", data=b"foo")
    assert response.status_code == 504
    response = client.get("/metrics")
    assert response.status_code == 200
    assert (
        'request_latency_seconds_count{cache="OFFLINE_MISS",'
        'method="POST",status="504",user="None"} 1.0'
    ) in str(response.data)
...
def test_rate_limited(_mock_monitor_session, mock_request, client):
    setup_mocked_requests_session_get(
        _mock_monitor_session, mocked_requests_monitor_good
    )
    ...
    assert response.status_code == 200
    assert (
        'request_latency_seconds_count{cache="RATE_LIMITED_MISS",'
        'method="GET",status="403",user="None"} 1.0'
    ) in str(response.data)
    ...
    assert response.status_code == 200
    assert (
        'request_latency_seconds_count{cache="RATE_LIMITED_HIT",'
        'method="GET",status="200",user="None"} 1.0'
    ) in str(response.data)


mocked_requests_pagination = [
    MockResponse("", {"ETag": "foo"}, 200, json_content=[{"a": "b"}, {"c", "d"}]),
    MockResponse(
        "",
        {
            "Link": '<https://api.github.com/blabla?page=2>; rel="next", <https://api.github.com/blabla?page=3>; rel="last"'
        },
        304,
        json_content=[{"a": "b"}, {"c", "d"}],
    ),
    MockResponse("", {}, 304, json_content=[{"a": "b"}, {"c", "d"}]),
]
...
@mock.patch(
    "ghmirror.core.mirror_requests.requests.request",
    side_effect=mocked_requests_pagination,
)
@mock.patch("ghmirror.data_structures.monostate.requests.Session")
def test_pagination_corner_case(mock_monitor_session, _mock_get, client):
    ...
    # 2nd request is a hit, but it contains pagination data
    # Ensure the pagination data is updated.
    response = client.get("/repos/app-sre/github-mirror", follow_redirects=True)
    assert response.status_code == 200
    ...
    # 3nd request is a hit, but it DOES NOT contain pagination data
    # Ensure the pagination data is updated.
    response = client.get("/repos/app-sre/github-mirror", follow_redirects=True)
    assert response.status_code == 200
...
def test_mirror_request_timeout_hit(mock_monitor_session, mock_get, client):
    ...
    assert response.status_code == 200
    assert (
        'request_latency_seconds_count{cache="API_TIMEOUT_HIT",'
        'method="GET",status="200",user="None"} 1.0'
    ) in str(response.data)
    assert (
        'request_latency_seconds_count{cache="ONLINE_MISS",'
...
def test_mirror_request_5xx(mock_monitor_session, mock_get, client):
    ...
    assert response.status_code == 200
    assert (
        'request_latency_seconds_count{cache="API_ERROR_HIT",'
        'method="GET",status="200",user="None"} 1.0'
    ) in str(response.data)
    assert (
        'request_latency_seconds_count{cache="ONLINE_MISS",'
...
def test_mirror_request_5xx_miss(mock_monitor_session, mock_get, client):
    ...
    mock_get.side_effect = mocked_requests_get_error
    response = client.get("/repos/app-sre/github-mirror/2", follow_redirects=True)
    assert response.status_code == 500
    ...
    assert response.status_code == 200
    assert (
        'request_latency_seconds_count{cache="API_ERROR_MISS",'
        'method="GET",status="500",user="None"} 1.0'
    ) in str(response.data)
    assert (
        'request_latency_seconds_count{cache="ONLINE_MISS",'
        'method="GET",status="200",user="None"} 1.0'
...
def test_mirror_request_connection_error_hit(mock_monitor_session, mock_get, client):
    ...
    assert response.status_code == 200
    assert (
        'request_latency_seconds_count{cache="API_CONNECTION_ERROR_HIT",'
        'method="GET",status="200",user="None"} 1.0'
    ) in str(response.data)
    assert (
        'request_latency_seconds_count{cache="ONLINE_MISS",'
